...
src_prepare() {
	epatch "${FILESDIR}/remove-toolset-1.46.0.patch"
	epatch "${FILESDIR}/boost-1.46.0-lambda_bind.patch"

	# This enables building the boost.random library with /dev/urandom support
	if [[ -e /dev/urandom ]] ; then
		mkdir -p libs/random/build
		cp "${FILESDIR}/random-Jamfile-1.46.0" libs/random/build/Jamfile.v2
	fi
}
