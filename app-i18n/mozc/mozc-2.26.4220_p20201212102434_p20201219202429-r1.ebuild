...
if [[ "${PV}" == "9999" ]]; then
	...
	SRC_URI="https://github.com/google/${PN}/archive/${MOZC_GIT_REVISION}.tar.gz -> ${PN}-${PV%%_p*}-${MOZC_DATE}.tar.gz
		https://github.com/abseil/abseil-cpp/archive/${ABSEIL_VERSION}.tar.gz -> abseil-${ABSEIL_VERSION}.tar.gz
...
src_unpack() {
	if [[ "${PV}" == "9999" ]]; then
		...
		unpack abseil-${ABSEIL_VERSION}.tar.gz
		rmdir ${P}/src/third_party/abseil-cpp || die
		mv abseil-cpp-${ABSEIL_VERSION} ${P}/src/third_party/abseil-cpp || die
